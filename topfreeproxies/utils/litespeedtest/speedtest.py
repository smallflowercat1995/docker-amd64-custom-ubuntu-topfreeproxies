...
def speedtest(subscription,output_range,other_config={'concurrency': -1, 'timeout': -1}):
    ...
    if os.name == 'posix':
        args = ['./lite', '--config', 'config.json', '--test', '../../sub/sub_merge_base64.txt']
    ...
    if ',' in output_range:
        output_range = {'begin': output_range.split(',',1)[0],'end': output_range.split(',',1)[1]}
    elif output_range == '-1':
        output_range = {'begin': 0, 'end': len(proxies_all['nodes'])}
    else:
        output_range = {'begin': 0, 'end': int(output_range)}
    for index in range(output_range['begin'],output_range['end']):
        try:
            proxy = proxies_all['nodes'][index]['link']
...
def progressbar(current,range,desc,size=30):
    empty='·'
    increment = 'C'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', '-p', help='Output file path', default='../../temp')
